...
```
  ...
  That's much better. Now that we have our data, what exactly do we wish to do with it? 
  
## Part 2 - Exploratory Data Analysis (EDA)

  For most people, the most common criticisms of fast food are fat content, sodium, and calorie count. Let's try finding out the average values for these attributes for all food categories offered by McDonald's. 
  
  First, let's use the select function to grab only the attributes we care about. 
  
```{r menu_select}
menu_averages <- menu %>%
  select(category, item, calories, total_fat, sodium, total_fat_percent_daily_value, sodium_percent_daily_value)
