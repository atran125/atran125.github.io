...
menu <- menu %>%
  mutate(`Ratio of Fat to Calories`= `Calories from Fat`/`Calories`)

menu %>%
  ggplot(aes(x=`Total Fat (g)`, y = `Ratio of Fat to Calories`)) + geom_point() + ggtitle("Contribution to Calories via Total Fat") + geom_smooth()
```
 

Oooooo. Now this is quite intriguing. We can see that, regardless of menu item category, as the amount of fat in a menu item increases, the more its Calories are due to the amount of fat it contains. It seems that a large number of menu items tend to however around the 0.3 - 0.4 range. But what's that there, at the very far right of the graph? Is there really some item on the McDonald's menu that actually possesses over 120 grams of fat? What on earth?
...
menu %>%
  ...
  slice(1:10) %>%
  ggplot(aes(x=Item, y =`Total Fat (g)`)) + geom_bar(stat="identity") + ggtitle("McDonald's Most Fattiest Menu Items") + coord_flip()
