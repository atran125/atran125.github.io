...
```{r test}
test_menu <- select(menu, Category, Item, `Trans Fat`)
rename(test_menu, `Trans Fat (g)`=`Trans Fat`)
```
...
menu_averages <- menu %>%
  ...
  summarize(`Mean Calories`=mean(Calories),
            `Mean Sodium (mg)`=mean(`Sodium (mg)`),
            `Mean Total Fat (g)` = mean(`Total Fat (g)`),
            `Mean Sugars (g)` = mean(`Sugars (g)`)) 
...
# plot mean calories 
menu_averages %>%
  ggplot(mapping=aes(x=Category, y=`Mean Calories`)) +
    geom_bar(aes(fill=Category), stat="identity") + ggtitle("Mean Calories vs. Menu Category") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) 

# plot mean sodium 
menu_averages %>%
  ggplot(mapping=aes(x=Category, y=`Mean Sodium (mg)`)) +
    geom_bar(aes(fill=Category), stat="identity") + ggtitle("Mean Sodium (mg) vs. Menu Category") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) 

# plot mean total fat  
menu_averages %>%
  ggplot(mapping=aes(x=Category, y=`Mean Total Fat (g)`)) +
    geom_bar(aes(fill=Category), stat="identity") + ggtitle("Mean Total Fat (g) vs. Menu Category") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) 

# plot mean sugars
menu_averages %>%
  ggplot(mapping=aes(x=Category, y=`Mean Sugars (g)`)) +
    geom_bar(aes(fill=Category), stat="identity") + ggtitle("Mean Sugars (g) vs. Menu Category") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) 
```
  

  This looks much nicer, and is easier to convey information to a viewer. But what exactly can we conclude? 
We can clearly see that the top two offending categories in terms of 
